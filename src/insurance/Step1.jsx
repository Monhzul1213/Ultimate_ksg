...
function setValue(form, key, value, type = "") {
  if (type === "CHECK") form.setFieldsValue({ [key]: value === "Y" });
  else if (type === "DATE" || key.includes("Date")) {
    if (value) form.setFieldsValue({ [key]: moment(value, dateFormat) });
    else form.setFieldsValue({ [key]: null });
  } else form.setFieldsValue({ [key]: value });
}

function getValue(obj, key, value) {
  if (typeof value === "boolean") obj[key] = value ? "Y" : "N";
  else if (isMoment(value)) obj[key] = moment(value).format(dateFormat);
  else if (typeof value !== "undefined") obj[key] = value;
}
...
class LicenceInput extends React.Component {
  ...
  onBlur = () => {
    ...
    if (!mask.test(value && value.toUpperCase())) onChange("");
    if (onBlur) {
      onBlur();
    }
  };
  ...
  onChangeRegNo = (e) => {
    ...
    if (this.fieldRegNo && value) {
      ...
      if (reg.test(value)) {
        ...
        const date = new Date(
          month > 20 ? year + 2000 : year + 1900,
          month > 20 ? month - 20 : month - 1,
          day
        );

        const pBDate = this.relatedProperties.find(
          (property) => property.RelatedObject === "isCustomer.BDate"
        );
        const pAge = this.relatedProperties.find(
          (property) => property.RelatedObject === "isCustomer.Age"
        );
        const pGender = this.relatedProperties.find(
          (property) => property.RelatedObject === "isCustomer.Gender"
        );
            ...
            if (relatedObject && relatedObject.length > 0) {
              ...
              this.relatedProperties
                ...
                .forEach((property) => {
                  setValue(
                    this.props.form,
                    "n" + property.PropertyID,
                    relatedObject[0][1][0][
                      property.RelatedObject.replace(
                        relatedObject[0][0] + ".",
                        ""
                      )
                    ],
        ...
        switch (property.RelatedObject) {
          ...
          case "isCustomer.CountryType":
            return (
              ...
              >
                {baseData &&
                  baseData.countryType &&
                  baseData.countryType.map((item) => (
                    <Option key={item.ConstKey}>{item.ValueStr1}</Option>
                  ))}
              </Select>
            );
        ...
        <Spin indicator={loadingIcon} spinning={this.state.loading}>
          <Form autoComplete="off" labelAlign="left">
            {productProperty &&
              ...
              productProperty.properties.map((property) => {
                const { getFieldDecorator } = form;
                const value =
                  bufferData &&
                  bufferData.length > 0 &&
                  bufferData[0]["n" + property.PropertyID];
                return (
                  <Form.Item
                    key={property.PropertyID}
                    style={{ marginBottom: "0px" }}
                    label={
                      property.DataType === "CHECK" ? "" : property.ColumnHeader
                    }
                  >
                    {getFieldDecorator("n" + property.PropertyID, {
                      rules: [
                        {
                          required: property.isRequired === "Y",
                          message: "Мэдээлэл оруулах шаардлагатай.",
                        },
                      ],
                      valuePropName:
                        property.DataType === "CHECK" ? "checked" : "value",
                      initialValue:
          ...
          <Form autoComplete="off" labelAlign="left">
            ...
            <Form.Item label="Регистрийн дугаар" {...formItemLayout}>
              {getFieldDecorator("CustID", {
                initialValue: Customer && Customer[0].CustID,
                rules: required,
              })(<Input />)}
            </Form.Item>
            <Form.Item label="Овог" {...formItemLayout}>
              {getFieldDecorator("LName", { rules: required })(<Input />)}
            </Form.Item>
            <Form.Item label="Нэр" {...formItemLayout}>
              {getFieldDecorator("Name", {
                initialValue: Customer && Customer[0].Name,
                rules: required,
              })(<Input />)}
            </Form.Item>
...
const ContractItemForm = Form.create({ name: "contractitem_form" })(
  ModalContractItemForm
);
...
class Step1 extends React.Component {
  ...
  getData = (ContractNo) => {
    ...
    request
      ...
      .then((res) => {
        ...
        const isGZInsurance =
          this.state.baseData &&
          this.state.baseData.isSetup &&
          this.state.baseData.isSetup.length > 0 &&
          this.state.baseData.isSetup[0].GZInsurance ===
            isContractItem[0].ProductID;
        if (!isGZInsurance) {
        ...
        Object.keys(getFieldsValue()).forEach((key) => {
          switch (key) {
            ...
            default:
              ...
              } else values[key] = isContract[0][key];
          }
        });
  ...
  saveFormData = () => {
    const { form, setTestData } = this.props;
    setTestData({
      ContractNo: form.getFieldValue("ContractNo"),
      Type: form.getFieldValue("Type"),
      Site: form.getFieldValue("SiteID"),
      Employee: form.getFieldValue("ContractEmpCode"),
      CustID: form.getFieldValue("CustID"),
      CustName: form.getFieldValue("CustName"),
      Product: form.getFieldValue("ProductID"),
      ...
      AgentPayPercent: form.getFieldValue("AgentPayPercent"),
    });
  };

  next = () => {
    const { next, testData } = this.props;
    this.saveFormData();
    next();
  };
  ...
  calcGZDAmount(values) {
    ...
    if (isGZDType) {
      ...
      isGZDType.forEach((item) => {
        if (
          item.CountryType == values.CountryType &&
          item.Limit == values.Limit &&
          item.Period == values.Days
        ) {
          if (values.PackageType === "O") {
            if (values.InsType === "INS1") GZDPackageAmt = item.Individual2;
            if (
              values.InsType === "INS2" ||
              values.InsType === "INS3" ||
              values.InsType === "INS4"
            ...
            if (values.InsType === "INS1") GZDPackageAmt = item.Family2;
            if (
              values.InsType === "INS2" ||
              values.InsType === "INS3" ||
              values.InsType === "INS4"
  ...
  onPressEnterCustomer = (e) => {
    ...
    if (e.target.value) {
      ...
      request
        .post("ContractSheet_GetCustomer", {
          token: this.state.LoggedSysuser.token,
          CustID,
          Name,
        })
  ...
  onOkItem = () => {
    ...
    form.validateFieldsAndScroll({ first: true }, (err, values) => {
      ...
      if (im === BaseInvoiceMode.View) im = BaseInvoiceMode.Edit;
  ...
  onOkCustomer = () => {
    ...
    form.validateFieldsAndScroll({ first: true }, (err, values) => {
      ...
      this.setState({ loading: true });
      request
        .post("ContractSheet_ModifyCustomer", {
          token: this.state.LoggedSysuser.token,
          json: JSON.stringify({ arCustomer: [values] }),
        })
        .then((res) => {
          if (res.data.retType !== 0) {
            this.setState({ loading: false });
  ...
  onChangeActualAmt = (value) => {
    ...
    var premiumAmt =
      ((value * values.BasePremiumAmt) / 100) *
      (1 - values.DiscountPercent / 100);
  ...
  onChangeBasePremiumAmt = (value) => {
    ...
    var premiumAmt =
      ((values.ActualAmt * value) / 100) * (1 - values.DiscountPercent / 100);
    this.setFieldsValue({ PremiumAmt: premiumAmt });
    this.onChangeControls();
  };
  ...
  onChangeDiscountPercent = (value) => {
    ...
    var premiumAmt =
      ((values.ActualAmt * values.BasePremiumAmt) / 100) * (1 - value / 100);
    this.setFieldsValue({ PremiumAmt: premiumAmt });
    this.onChangeControls();
  };
  ...
  onChangeProduct = (value) => {
    const isGZInsurance =
      ...
      this.state.baseData.isSetup &&
      this.state.baseData.isSetup.length > 0 &&
      this.state.baseData.isSetup[0].GZInsurance === value;
  ...
  onChangeGZDPackageAmt = (name, value) => {
    ...
    values[name] = value;
    this.setFieldsValue({
      ActualAmt: values.Limit * values.Rate,
      PremiumAmt: values.GZDPackageAmt * values.Rate,
    });
    this.onChangeControls();
  };
  ...
  render() {
    ...
    const { baseD } = this.props;
    const productID = getFieldValue("ProductID");
    const prod = this.getProduct(productID);
    const isGZInsurance =
      baseD &&
      baseD.isSetup &&
      baseD.isSetup.length > 0 &&
      baseD.isSetup[0].GZInsurance === productID;
    const isGZDType =
      isGZInsurance &&
      baseD &&
      baseD.isGZDType &&
      baseD.isGZDType.filter(
        (value) => value.CountryType === getFieldValue("CountryType")
      );
      ...
      <Spin indicator={loadingIcon} spinning={this.state.loading}>
        <Form {...formLayount}>
          ...
          <Form.Item label="Төрөл" {...formItemLayout}>
            {getFieldDecorator("Type", {
              initialValue: testData.Type || "N",
              rules: required,
            })(
              <Select onChange={this.onChangeControls} disabled={disabledEdit}>
                {baseD &&
                  baseD.isContract_Type &&
                  baseD.isContract_Type.map((iscontract_type) => (
                    <Option key={iscontract_type.ConstKey}>
                      {iscontract_type.ValueStr1}
                    </Option>
                  ))}
              </Select>
            )}
          ...
          <Form.Item label="Валют" {...formItemLayout}>
            {getFieldDecorator("CurrencyID", {
              initialValue: testData.Currency || "MNT",
              rules: required,
            })(
              <Select onChange={this.onChangeControls} disabled={disabledEdit}>
                {baseD &&
                  baseD.Currency &&
                  baseD.Currency.map((currency) => (
                    <Option key={currency.CurrencyID}>
                      {currency.CurrencyID}
                    </Option>
                  ))}
              </Select>
          ...
          <Form.Item label="Салбар" {...formItemLayout}>
            {getFieldDecorator("SiteID", {
              initialValue:
                ...
                (baseD &&
                  baseD.ContractSite &&
                  baseD.ContractSite.length > 0 &&
                  baseD.ContractSite[0].SiteID),
              rules: required,
            })(
              <Select
                showSearch
            ...
            <Row gutter={8}>
              <Col span={20}>
                {getFieldDecorator("ProductID", {
                  initialValue: testData.Product || null,
                  rules: required,
                })(
                  <Select
              ...
              <Col span={4} style={{ textAlign: "right" }}>
                <Button
                  ...
                  icon="form"
                  disabled={disabledEdit}
                  onClick={this.onClickProductButton}
                  className="ButtonStyle"
                />
              </Col>
            </Row>
          ...
          <Form.Item label="Мэргэжилтэн" {...formItemLayout}>
            {getFieldDecorator("ContractEmpCode", {
              initialValue:
                testData.Employee || this.state.LoggedSysuser.EmpCode,
            })(
              <Select onChange={this.onChangeControls} disabled={disabledEdit}>
                {baseD &&
                  baseD.Emp &&
          ...
          <Form.Item label="Төлөөлөгч / Зуучлагч" {...formItemLayout}>
            {getFieldDecorator("AgentID", {
              initialValue:
                ...
                (baseD &&
                  baseD.EmpCodeDtl &&
                  baseD.EmpCodeDtl.length > 0 &&
                  baseD.EmpCodeDtl[0].RegistryNumber),
              rules: required,
            })(
              <Select
                showSearch
          ...
          <Form.Item label="Даатгуулагч" {...formItemLayout} required="true">
            <Form.Item {...formItemLayout}>
              {getFieldDecorator("CustID", {
                initialValue: testData.CustID || null,
                rules: required,
              })(
                <Input
                  onChange={this.onChangeControls}
            ...
            <Form.Item {...formItemLayout}>
              {getFieldDecorator("CustName", {
                initialValue: testData.CustName || null,
                rules: required,
              })(
                <Input
                  onChange={this.onChangeControls}
          ...
          <Form.Item label="Гэрээний огноо" {...formItemLayout}>
            {getFieldDecorator("ContractDate", {
              initialValue:
                testData.Contractdate || moment(this.state.today, dateFormat),
              rules: required,
            })(
              <DatePicker
                style={{ width: "100%" }}
          ...
          <Form.Item label="Дуусах огноо" {...formItemLayout}>
            {getFieldDecorator("EndDate", {
              initialValue: testData.EndDate || null,
              rules: required,
            })(
              <DatePicker
                style={{ width: "100%" }}
          ...
          <Form.Item label="Төлөв" {...formItemLayout}>
            {getFieldDecorator("Status", {
              initialValue: testData.Status || "H",
              rules: required,
            })(
              <Select onChange={this.onChangeControls} disabled={disabledEdit}>
                {baseD &&
                  baseD.isContractSheet_Status &&
                  baseD.isContractSheet_Status.map((isContractSheet_Status) => (
          ...
          <Form.Item label="Хураамж төлөх хэлбэр" {...formItemLayout}>
            {getFieldDecorator("PaymentType", {
              initialValue: testData.Paymenttype || "01",
              rules: required,
            })(
              <Select onChange={this.onChangeControls} disabled={disabledEdit}>
                {baseD &&
                  baseD.isContract_PaymentType &&
                  baseD.isContract_PaymentType.map((isContract_PaymentType) => (
                    <Option key={isContract_PaymentType.ConstKey}>
                      {isContract_PaymentType.ValueStr1}
                    </Option>
                  ))}
              </Select>
            )}
          ...
          <Form.Item label="Шимтгэлийн хувь (%)" {...formItemLayout}>
            {getFieldDecorator("AgentPayPercent", {
              initialValue: testData.AgentPayPercent || null,
            })(
              <InputNumber
                style={{ width: "100%" }}
                onChange={this.onChangeControls}
          ...
          <Form.Item {...tailFormLayout}>
            <Button disabled={disabledEdit} type="primary" onClick={this.next}>
              Дараах
            </Button>
          </Form.Item>
        </Form>
        {this.state.showItem && (
          <ContractItemForm
            ...
            productProperty={
              this.state.bufferData && this.state.bufferData.productProperty
            }
            baseD={
              this.props.baseData && {
                agentType: this.state.baseData.arCustomer_AgentType,
                countryType: this.state.baseData.isCustomer_CountryType,
                gender: this.state.baseData.hrEmp_Gender,
              }
            }
            bufferData={
              this.state.bufferData && this.state.bufferData.isContractItem
            }
...
const TestForm = Form.create()(Step1);
