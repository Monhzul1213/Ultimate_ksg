    ...
    return (
      <div style={{ margin: "40px", marginTop: "15px" }}>
        ...
        <Spin spinning={this.state.loading}>
          ...
          />
          <Card title="Олгох болон нэхэмжилэх дүн" size="small" style={{marginBottom: '8px'}}>
  ...
  render() {
    ...
    data &&
      data.reduce(function (res, value) {
        const month = value.IndemRegDate.slice(0, 7);
        if (!res[month]) {
          res[month] = { month, RequiredAmt: 0, SolvedAmt: 0 };
          result.push(res[month]);
        }
        res[month].RequiredAmt += value.RequiredAmt;
        res[month].SolvedAmt += value.SolvedAmt;
        return res;
      }, {});
