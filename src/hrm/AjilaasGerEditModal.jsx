...
class App extends React.Component {
  ...
  handleOk = (e) => {
    ...
    form.validateFields((errors, values) => {
      ...
      console.log(this.state.data.EmpDtl);
      this.state.data.EmpDtl[0].RowStatus = "U";
      request
        .post("Profile_Modify", {
          token: this.state.LoggedSysuser.token,
          json: JSON.stringify({
            BusinessObject: { ProfileType: "AjilaasGer" },
      ...
      <div>
        ...
        >
          <Spin spinning={this.state.loading} style={{ height: "100%" }}>
            <Form>
              <h3 style={{ marginBottom: "20px", fontSize: "13pt" }}>
                <b>
                  АЖИЛААС - ГЭР хүртэл(чиглэл, маршруут болон автобусны буудлын
                  нэр бичнэ)
                </b>
              </h3>
              <Row>
                <Form.Item {...formItemLayout}>
                  {getFieldDecorator("AjilaasGer", {
                    rules: required,
                    initialValue:
                      this.state.data &&
                      this.state.data.EmpDtl &&
                      this.state.data.EmpDtl.length > 0
                        ? this.state.data.EmpDtl[0].AjilaasGer
                        : "",
                  })(
                    <TextArea
                      size="large"
                      style={{ fontSize: "12pt", fontFamily: "Arial" }}
                      placeholder="АЖИЛААС - ГЭР хүртэл(чиглэл, маршруут болон автобусны буудлын нэр бичнэ)"
                    />
                  )}
                  <p
                    style={{
                  ...
                  >
                    Загварын текст: ХААН Тауэр/АЖИЛ/ - Номын сан - Багшийн дээд
                    - Бөхийн өргөө - 13-р хороолол/Гэр/
                  </p>
                </Form.Item>
              </Row>
              <Row>
                <Col {...columnConfig}>
                  <Form.Item {...formItemLayout}>
                    {getFieldDecorator("AjilaasGer_time", {
                      rules: required,
                      initialValue:
                        this.state.data &&
                        this.state.data.EmpDtl &&
                        this.state.data.EmpDtl.length > 0
                          ? this.state.data.EmpDtl[0].AjilaasGer_time
                          : "",
                    })(
                      <Input
                        size="large"
