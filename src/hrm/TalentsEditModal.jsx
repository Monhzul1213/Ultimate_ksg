...
class App extends React.Component {
      ...
      if (!err) {
        ...
        request
          .post("Profile_Modify", {
            ...
            json: JSON.stringify({
              BusinessObject: { ProfileType: "Skill" },
              Skill: names,
            }),
          })
          .then((res) => {
  ...
  render() {
    ...
    const data = {
      keys:
        this.state.data && this.state.data.Skill
          ? this.state.data.Skill.map((item, index) => index)
          : [],
      names:
        this.state.data && this.state.data.Skill ? this.state.data.Skill : [],
    };
      ...
      return (
        ...
        >
          <Card key={k} style={{ margin: "2px", marginTop: "20px" }}>
            ...
            <Row gutter={30}>
              <Col {...columnConfig}>
                {getFieldDecorator(`names[${k}].SkillTypeID`, {
                  initialValue:
                    data.names && data.names[k] && data.names[k].SkillTypeID,
                  ...
                  >
                    {this.state.data &&
                      this.state.data.SkillType &&
                      this.state.data.SkillType.map((item, index) => (
                        <Option key={item.SkillTypeID}>{item.Descr}</Option>
                      ))}
                  </Select>
                )}
              </Col>
              <Col {...columnConfig}>
                {getFieldDecorator(`names[${k}].LevelValue`, {
                  initialValue:
                    data.names && data.names[k] && data.names[k].LevelValue,
                  rules: RowStatus === "D" ? [] : required,
                })(
                  <Input
                    placeholder="Түвшин"
            ...
            <Row gutter={[30, 35]} style={{ marginTop: "4px" }}>
              <Col>
                {getFieldDecorator(`names[${k}].Note`, {
                  ...
                  <Input
                    placeholder="Чадварын тухай мэдээлэл"
                    size="large"
                    style={{ fontSize: "13pt", fontFamily: "Arial" }}
                    onChange={() => {
                      const RowStatus = this.props.form.getFieldValue(
...
export default class TalentsEdit extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
