...
class App extends React.Component {
      ...
      if (!err) {
        ...
        request
          .post("Profile_Modify", {
            ...
            json: JSON.stringify({
              BusinessObject: { ProfileType: "Illness" },
              Illness: names,
            }),
          })
          .then((res) => {
  ...
  render() {
    ...
    const data = {
      keys:
        this.state.data && this.state.data.Illness
          ? this.state.data.Illness.map((item, index) => index)
          : [],
      names:
        this.state.data && this.state.data.Illness
          ? this.state.data.Illness
          : [],
    };
        ...
        <Card key={k} style={{ margin: "2px", marginTop: "20px" }}>
          ...
          >
            ...
            <Row gutter={[25, 30]} type="flex">
              <Col {...columnConfig}>
                {getFieldDecorator(`names[${k}].IllnessTypeID`, {
                  initialValue:
                    data.names && data.names[k] && data.names[k].IllnessTypeID,
                  rules: RowStatus === "D" ? [] : required,
                })(
                  <Select
                    placeholder="Өвчний код"
                    size="large"
                  ...
                  >
                    {this.state.data &&
                      this.state.data.IllnessType &&
                      this.state.data.IllnessType.map((item, index) => (
                        <Option key={item.IllnessTypeID}>
                          {item.IllnessTypeID} {item.Descr}
                        </Option>
                      ))}
                  </Select>
                )}
              ...
              <Col {...columnConfig}>
                {getFieldDecorator(`names[${k}].IllnessValue`, {
                  initialValue:
                    data.names && data.names[k] && data.names[k].IllnessValue,
                  rules: RowStatus === "D" ? [] : required,
                })(
                  <Input
                    placeholder="Утга"
...
export default class IllnessEditModal extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
